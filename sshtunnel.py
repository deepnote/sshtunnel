    ...
    **SSH tunnel class**
    ...
    def _get_binds(bind_address, bind_addresses, is_remote=False):
        ...
        if not bind_address and not bind_addresses:
            if is_remote:
                raise ValueError(
                    "No {0} bind addresses specified. Use '{0}_bind_address' or '{0}_bind_addresses' argument".format(
                        addr_kind
                    )
                )
            else:
            ...
            raise ValueError(
                "You can't use both '{0}_bind_address' and '{0}_bind_addresses' arguments. Use one of them.".format(
                    addr_kind
                )
            )
        if bind_address:
