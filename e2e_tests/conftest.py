...
def e2e_infrastructure():
    ...
    # Keep a host-side copy of the private key that the container can't touch.
    # The container's init chowns bind-mounted files to PUID/PGID, which makes
    # the originals unreadable by the CI runner (different UID).
    host_key_dir = Path(tempfile.mkdtemp(prefix="sshtunnel-e2e-hostkey-"))
    host_private_key = host_key_dir / "ssh_host_rsa_key"
    shutil.copy2(private_key_path, host_private_key)
    ...
    try:
        ...
        shutil.rmtree(host_key_dir, ignore_errors=True)
